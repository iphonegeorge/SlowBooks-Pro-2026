...
def csv_export_customers(db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
...
def csv_export_vendors(db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
...
def csv_export_items(db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
...
def csv_export_accounts(db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
...
async def csv_import_customers(file: UploadFile = File(...), db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
...
async def csv_import_vendors(file: UploadFile = File(...), db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    content = (await file.read()).decode("utf-8-sig")
...
async def csv_import_items(file: UploadFile = File(...), db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    content = (await file.read()).decode("utf-8-sig")
